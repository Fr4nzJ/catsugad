...
<div class="container" style="margin-top: 2rem; margin-bottom: 4rem;">
    ...
    <div style="margin-bottom: 2rem;">
        <h1 style="margin: 0; color: #333; font-size: 2rem;">
            <i class="fas fa-map-pin"></i> Edit Map Marker
        </h1>
        <p style="margin: 0.5rem 0 0 0; color: #666; font-size: 0.95rem;">Update location marker details and position</p>
    </div>

    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 2rem;">
        <!-- Form Section -->
        <div style="background: white; padding: 2rem; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
            <form action="{{ route('admin.map-markers.update', $mapMarker) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div style="margin-bottom: 1.5rem;">
                    ...
                    <input type="text" name="name" value="{{ old('name', $mapMarker->name) }}" placeholder="e.g., Faculty Center" 
                ...
                <div style="margin-bottom: 1.5rem;">
                    ...
                    <input type="number" name="latitude" step="0.000001" value="{{ old('latitude', $mapMarker->latitude) }}" 
                        style="width: 100%; padding: 0.75rem; border: 1px solid #ddd; border-radius: 4px; font-size: 1rem; box-sizing: border-box;"
                        required>
                    @error('latitude')
                        <span style="color: #e74c3c; font-size: 0.85rem; margin-top: 0.25rem; display: block;">{{ $message }}</span>
                ...
                <div style="margin-bottom: 1.5rem;">
                    ...
                    <input type="number" name="longitude" step="0.000001" value="{{ old('longitude', $mapMarker->longitude) }}" 
                        style="width: 100%; padding: 0.75rem; border: 1px solid #ddd; border-radius: 4px; font-size: 1rem; box-sizing: border-box;"
                        required>
                    @error('longitude')
                        <span style="color: #e74c3c; font-size: 0.85rem; margin-top: 0.25rem; display: block;">{{ $message }}</span>
                ...
                <div style="margin-bottom: 1.5rem;">
                    ...
                    <textarea name="description" rows="3" placeholder="Enter location description..." 
                        style="width: 100%; padding: 0.75rem; border: 1px solid #ddd; border-radius: 4px; font-size: 1rem; box-sizing: border-box; font-family: inherit;">{{ old('description', $mapMarker->description) }}</textarea>
                    @error('description')
                        <span style="color: #e74c3c; font-size: 0.85rem; margin-top: 0.25rem; display: block;">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div style="margin-bottom: 1.5rem;">
                    ...
                    <select name="page" style="width: 100%; padding: 0.75rem; border: 1px solid #ddd; border-radius: 4px; font-size: 1rem; box-sizing: border-box;" required>
                        ...
                        <option value="contact" {{ old('page', $mapMarker->page) === 'contact' ? 'selected' : '' }}>Contact Page</option>
                        <option value="programs" {{ old('page', $mapMarker->page) === 'programs' ? 'selected' : '' }}>Programs Page</option>
                    </select>
                    @error('page')
                        <span style="color: #e74c3c; font-size: 0.85rem; margin-top: 0.25rem; display: block;">{{ $message }}</span>
                    @enderror
                ...
                <div style="margin-bottom: 2rem;">
                    <label style="display: flex; align-items: center; gap: 0.75rem; cursor: pointer;">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $mapMarker->is_active) ? 'checked' : '' }} 
        ...
        <div style="background: white; padding: 2rem; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
            <h3 style="margin-top: 0; margin-bottom: 1rem; color: #333;">
                <i class="fas fa-map"></i> Click on map to update marker position
            </h3>
            ...
            <p style="margin: 1rem 0 0 0; color: #666; font-size: 0.9rem;">
                <strong>Tip:</strong> Click anywhere on the map to move the marker. The latitude and longitude will update automatically.
            </p>
        </div>
    </div>
</div>
...
<script>
    ...
    // Initialize map
    const map = L.map('map').setView([{{ old('latitude', $mapMarker->latitude) }}, {{ old('longitude', $mapMarker->longitude) }}], 15);
    ...
    let marker = L.marker([{{ old('latitude', $mapMarker->latitude) }}, {{ old('longitude', $mapMarker->longitude) }}])
        .addTo(map);
