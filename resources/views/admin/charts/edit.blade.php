...
<head>
    ...
    <title>Edit Chart - GAD CatSU Admin</title>
            ...
            <div class="column is-9">
                <div class="content-box">
                    <div class="page-header">
                        <h2><i class="fas fa-edit"></i> Edit Chart</h2>
                    </div>
                    ...
                    <form action="{{ route('admin.charts.update', $chart) }}" method="POST" novalidate id="chartForm">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="field">
                            ...
                            <div class="control">
                                <input class="input @error('name') is-danger @enderror" type="text" name="name" value="{{ old('name', $chart->name) }}" placeholder="e.g., Annual Participation Growth" required>
                            </div>
                            @error('name')
                            ...
                            <div class="control">
                                <div class="select @error('type') is-danger @enderror">
                                    <select name="type" id="chartType" required>
                                        ...
                                        <option value="growth" {{ old('type', $chart->type) === 'growth' ? 'selected' : '' }}>Growth Chart (Line Chart)</option>
                                        <option value="distribution" {{ old('type', $chart->type) === 'distribution' ? 'selected' : '' }}>Distribution Chart (Doughnut Chart)</option>
                                    </select>
                                </div>
                            </div>
                            @error('type')
                            ...
                            <div style="overflow-x: auto;">
                                <table class="table is-fullwidth is-hoverable" style="margin-bottom: 1rem;">
                                    ...
                                    <tbody id="dataTableBody">
                                        @php
                                            $labels = old('labels', $chart->labels) ?? [];
                                            $data = old('data', $chart->data) ?? [];
                                            if (is_string($labels)) $labels = json_decode($labels, true);
                                            if (is_string($data)) $data = json_decode($data, true);
                                        @endphp
                                        @forelse(array_combine($labels, $data) as $label => $value)
                                            <tr class="data-row">
                                                <td><input class="input" type="text" placeholder="e.g., Year 2020" value="{{ $label }}"></td>
                                                <td><input class="input" type="number" placeholder="e.g., 150" step="0.01" value="{{ $value }}"></td>
                        ...
                        <div class="field">
                            ...
                            <div class="control">
                                <input class="input @error('order') is-danger @enderror" type="number" name="order" value="{{ old('order', $chart->order) }}" min="0">
                            </div>
                            @error('order')
                                <p class="help is-danger">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="field is-grouped">
                            <div class="control">
                                <button type="submit" class="button is-success">Update Chart</button>
                            </div>
                            <div class="control">
