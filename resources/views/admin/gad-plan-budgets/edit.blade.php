...
@section('content')
<div class="container">
    <h1 class="title">Edit GAD Plan & Budget</h1>

    <form action="{{ route('admin.gad-plan-budgets.update', $gadPlanBudget) }}" method="POST" class="box">
        @csrf
        @method('PUT')
                ...
                <div class="field">
                    ...
                    <div class="control">
                        <input type="text" name="title" class="input @error('title') is-danger @enderror" 
                               value="{{ old('title', $gadPlanBudget->title) }}" required>
                    </div>
                    @error('title')
                        ...
                        <div class="select is-fullwidth @error('college_id') is-danger @enderror">
                            <select name="college_id" required>
                                ...
                                @foreach($colleges as $college)
                                    <option value="{{ $college->id }}" {{ old('college_id', $gadPlanBudget->college_id) == $college->id ? 'selected' : '' }}>
                                        {{ $college->name }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="field">
                    ...
                    <div class="control">
                        <input type="text" name="program_project" class="input @error('program_project') is-danger @enderror" 
                               value="{{ old('program_project', $gadPlanBudget->program_project) }}" required>
                    </div>
                ...
                <div class="field">
                    ...
                    <div class="control has-icons-left">
                        <input type="number" name="budget_amount" class="input @error('budget_amount') is-danger @enderror" 
                               value="{{ old('budget_amount', $gadPlanBudget->budget_amount) }}" step="0.01" min="0" required>
                ...
                <div class="field">
                    ...
                    <div class="control">
                        <textarea name="description" class="textarea @error('description') is-danger @enderror" 
                                  rows="4">{{ old('description', $gadPlanBudget->description) }}</textarea>
                    </div>
                    @error('description')
                        <p class="help is-danger">{{ $message }}</p>
                    @enderror
                ...
                <div class="field">
                    ...
                    <div class="control">
                        <input type="text" name="target_beneficiaries" class="input @error('target_beneficiaries') is-danger @enderror" 
                               value="{{ old('target_beneficiaries', $gadPlanBudget->target_beneficiaries) }}">
                    </div>
                ...
                <div class="field">
                    ...
                    <div class="control">
                        <input type="text" name="timeline" class="input @error('timeline') is-danger @enderror" 
                               placeholder="e.g., Q1 2026 - Q4 2026" value="{{ old('timeline', $gadPlanBudget->timeline) }}">
                    </div>
                    @error('timeline')
                        <p class="help is-danger">{{ $message }}</p>
                    @enderror
                    ...
                    <div class="control">
                        <div class="select is-fullwidth @error('status') is-danger @enderror">
                            <select name="status" required>
                                <option value="draft" {{ old('status', $gadPlanBudget->status) == 'draft' ? 'selected' : '' }}>Draft</option>
                                <option value="submitted" {{ old('status', $gadPlanBudget->status) == 'submitted' ? 'selected' : '' }}>Submitted</option>
                                <option value="approved" {{ old('status', $gadPlanBudget->status) == 'approved' ? 'selected' : '' }}>Approved</option>
                                <option value="rejected" {{ old('status', $gadPlanBudget->status) == 'rejected' ? 'selected' : '' }}>Rejected</option>
                            </select>
                        </div>
                    </div>
                    @error('status')
